...
class FakeTodoApi: TodoApi {
    ...
    override suspend fun getTodoList(token: String): GetTodoListResponseDTO {
        ...
        throw (HttpException(Response.error<LoginResponseDTO>(401, ResponseBody.create(MediaType.parse("plain/text"),"Invalid token"))))
    }
    ...
    override suspend fun addTodo(token: String, todo: AddEditTodoRequestDTO): Response<String> {
        if (token == "success") {
            return if (todo.todo.title == "success") {
                todoList.add(todo.todo)
                Response.success("New todo added")
            } else {
                Response.error(400, ResponseBody.create(MediaType.parse("plain/text"),"Failed to insert the new todo"))
            }
        }
        return Response.error(401, ResponseBody.create(MediaType.parse("plain/text"),"Invalid token"))
    }
    ...
    override suspend fun updateTodo(token: String, todo: AddEditTodoRequestDTO): Response<String> {
        if (token == "success") {
            ...
            return if (index != -1) {
                ...
                Response.error(400, ResponseBody.create(MediaType.parse("plain/text"),"Update failed"))
            }
        }
        return Response.error(401, ResponseBody.create(MediaType.parse("plain/text"),"Invalid token"))
    }
    ...
    override suspend fun deleteTodo(token: String, id: Int): Response<String> {
        if (token == "success") {
            return if (todoList.remove(todoList.find { it.id == id })) {
                ...
                Response.error(400, ResponseBody.create(MediaType.parse("plain/text"),"No todo with this id to delete for the connected user"))
            }
        }
        return Response.error(401, ResponseBody.create(MediaType.parse("plain/text"),"Invalid token"))
    }
